...
def run(
    ...
    model_dir: str = None,
    ### Interaction Item Column 지정 ###
    item: str = 'assessmentItemID',
):
    model.train()
    ...
    for e in range(n_epochs):
        ...
        if auc > best_auc:
            ...
            ### Embedding Save ###
            np.save(f"./embedding/embedding_{item}.npy", model.get_embedding(train_data['edge']).detach().cpu().numpy())
...
def inference(model: nn.Module, data: dict, output_dir: str):
    ...
    ### 한국 시간 활용해서 파일명 지정, 예측 파일 생성 ###
    KST = timezone(timedelta(hours=9))
    record_time = datetime.now(KST)
    write_path = os.path.join(output_dir, f"lgcn_submission_{record_time.strftime('%Y-%m-%d_%H-%M-%S')}.csv")
